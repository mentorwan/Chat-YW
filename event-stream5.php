...
$results = $db->query('SELECT * FROM main.chat_history ORDER BY id ASC');
    ...
    $history[] = [ROLE => SYS, CONTENT => "You are AP Statistics teacher, you want to test your student learning capabilities on AP Statistics problems"];
    while ($row = $results->fetchArray()) {
        $history[] = [ROLE => USER, CONTENT => $row['human']];
    //$history[] = [ROLE => ASSISTANT, CONTENT => $row['ai']];
        $history[] = [ROLE => ASSISTANT, CONTENT => "Give me a question in AP Statistics test"];
    }
